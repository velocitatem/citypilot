"""Scrape article headlines from Hong Kong Free Press archive.

Crawls the HKFP archive, caches raw HTML pages locally, and returns a
DataFrame with columns: title, url, date, author.

Usage (from project root):
    uv run scripts/fetch_hkfp_headlines.py

Output is written to data/hk_hkfp_headlines.csv.
...
def _parse_articles(html: str) -> list[dict]:
    ...
    for art in soup.select("article"):
        ...
        auth_el = art.select_one(
            "[rel='author'], .author a, .byline a, .post-author a"
        )
        if not title_el:
            continue
        raw = (
...
def _fetch_page(n: int, cache_dir: Path, proxy: str | None, retries: int = 5) -> str:
    ...
    for attempt in range(retries):
        ...
        try:
            r = requests.get(
                url, headers={"User-Agent": _UA}, proxies=prx, timeout=25
            )
...
def fetch_headlines(
    ...
    oldest_cached = (
        ...
        if oldest_html_path.exists() and max_cached > 1
        else None
    )

    if oldest_cached and (date.today() - oldest_cached).days > 7:
        rate = max_cached / (date.today() - oldest_cached).days
    ...
    print(
        f"  upper={upper} | {len(cached)} cached | {len(uncached)} to fetch"
    ...
    if uncached:
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futs = {
                pool.submit(
                    _fetch_page, n, cache_dir, proxies[i % len(proxies)]
                ): n
...
if __name__ == "__main__":
    ...
    out = _P("data/hk_hkfp_headlines.csv")
    df = fetch_headlines()
